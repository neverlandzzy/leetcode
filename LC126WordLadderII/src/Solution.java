...
public class Solution {
    ...
    public static List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
        ...
        // 用HashSet操作比直接用List快，直接用List会TLE
        HashSet<String> dict = new HashSet<String>(wordList);
        // 这个HashMap用于记录每个点的邻居。如果没有这个HashMap，在dfs中要对每个点for整个distance中的key（也就是wordList），
        // 复杂度增加
        HashMap<String, List<String>> nextWords = new HashMap<String, List<String>>();
        HashMap<String, Integer> distance = new HashMap<String, Integer>();
        		
        bfs(endWord, dict, distance, nextWords);

        dfs(beginWord, endWord, distance, nextWords, result, list);
        return result;
    }
    
    // 从beginWord开始，dfs每个点，把与beginWord距离相差1的点加入到path。当beginWord == endWord时，把path加入
    // 到result中，即找到1条路径。
    private static void dfs(String beginWord, String endWord, HashMap<String, Integer> distance,  HashMap<String, List<String>> nextWords, List<List<String>> result, List<String> list) { 
    	...
    	if (beginWord.equals(endWord)) {
    		...
    		for (String s: nextWords.get(beginWord)) {
    			if (distance.containsKey(s) && distance.get(beginWord) == distance.get(s) + 1) {
    				dfs(s, endWord, distance, nextWords, result, list);
    			}
    		}
    	}
    ...
    // 返回所有点到终点的距离
    private static void bfs(String endWord, HashSet<String> wordList, HashMap<String, Integer> distance,  HashMap<String, List<String>> nextWords) {
    	...
    	while (!queue.isEmpty()) {
        	...
        	for (int i = 0; i < size; i++) {
        		...
        		for (String s: getNextWords(tmp, wordList)) {
        			
        			nextWords.get(tmp).add(s);
        			if (distance.containsKey(s)) {
        				continue;
        			}

        			distance.put(s, distance.get(tmp)+1);
        			queue.offer(s);
        		}
        	}
    ...
    private static List<String> getNextWords(String word, HashSet<String> wordList) {
    ...
    public static void main(String[] args) {
		...
		//bfs(begin, end, wordList, nextWords, distance);
