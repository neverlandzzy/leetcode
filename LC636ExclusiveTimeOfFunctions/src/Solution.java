...
public class Solution {
    ...
    public static int[] exclusiveTime(int n, List<String> logs) {
        ...
        for (String log: logs) {
        	...
        	if (array[1].equals("start")) {
        		if (!stack.isEmpty()) {
        			result[stack.peek()] += Integer.parseInt(array[2]) - pre;
        		}
        		stack.push(Integer.parseInt(array[0]));
        		pre = Integer.parseInt(array[2]);
        	} else {
        		result[stack.pop()] += Integer.parseInt(array[2]) - pre + 1;
    ...
    public static void main(String[] args) {
    	...
    	List<String> test = new ArrayList<>(Arrays.asList("0:start:0", "1:start:2", "1:end:5", "0:end:6"));
    	
		int[] result = exclusiveTime(2, test);
